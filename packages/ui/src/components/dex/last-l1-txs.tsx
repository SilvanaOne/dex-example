...
export default function LastL1Txs() {
  ...
  return (
    <div className="h-full p-1 flex flex-col">
      <h3 className="text-xs font-semibold mb-0.5 text-[#02c076]">
        Last L1 Transactions
      </h3>
      ...
      <div className="overflow-hidden max-h-[calc(100%-18px)] flex-1">
        {transactions.map((tx) => (
          <div
            key={tx.id}
            className="grid grid-cols-5 text-[9px] hover:bg-[#2a2e37] transition-colors"
          >
            <div className="truncate text-[#02c076]">
              {shortenString(tx.txHash, 4)}
            </div>
            <div>{tx.blockNumber}</div>
            <div className="truncate text-[#848e9c]">
              {shortenString(tx.dataAvailabilityHash, 4)}
            </div>
