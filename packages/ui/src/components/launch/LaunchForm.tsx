...
export function LaunchForm({
  ...
  const [prepareDelay, setPrepareDelay] = useState<number | undefined>(
    undefined
  );
  const [executeDelay, setExecuteDelay] = useState<number | undefined>(
    undefined
  );
  const [indexingDelay, setIndexingDelay] = useState<number | undefined>(
    undefined
  );
  const [txError, setTxError] = useState<string | undefined>(undefined);
  const [appliedDigest, setAppliedDigest] = useState<string | undefined>(
    undefined
  );
  ...
  const [processing, setProcessing] = useState<ProcessingType | undefined>(
    undefined
  );
    ...
    <section className="relative py-24 dark:bg-jacarta-800">
      ...
      <div className="container">
        ...
        <div className="mx-auto max-w-[48.125rem] md:flex mt-8">
          ...
          <>
            <div
              className={`min-w-60 max-w-md rounded-2lg border ${
                highlight ? "border-accent" : "border-jacarta-100"
              } bg-white p-8 dark:border-jacarta-600 dark:bg-jacarta-700 ${
                highlight ? "shadow-accent-volume" : ""
              }`}
            >
